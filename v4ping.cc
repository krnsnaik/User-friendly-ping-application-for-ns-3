...
namespace ns3 {
    ...
    TypeId V4Ping::GetTypeId (void)
    {
        static TypeId tid = TypeId ("ns3::V4Ping")
            ...
            .AddAttribute ("Verbose",
                           ...
                           MakeBooleanChecker ())
            .AddAttribute ("Quiet",
                           "Produce only summary lines",
                           BooleanValue (false),
                           MakeBooleanAccessor (&V4Ping::m_quiet),
                           MakeBooleanChecker ())
            .AddAttribute ("Interval", "Wait  interval  seconds between sending each packet.",
                           TimeValue (Seconds (1)),
                           MakeTimeAccessor (&V4Ping::m_interval),
                           MakeTimeChecker ())
            .AddAttribute ("Count", "Send count of no of packets .",
                           UintegerValue (0),
    ...
    uint32_t V4Ping::GetApplicationId (void) const
    {
        NS_LOG_FUNCTION (this);
    ...
    void V4Ping::Receive (Ptr<Socket> socket)
    {
        ...
        while (m_socket->GetRxAvailable () > 0)
        {
            ...
            if(m_ttl>0)
                ipv4.SetTtl (m_ttl); //-t functionality
    ...
    void V4Ping::Send ()
    {
        ...
        else if(--m_count>0)            //-c functionality
            m_next = Simulator::Schedule (m_interval, &V4Ping::Send, this);
        delete[] data;
    }
    ...
    void V4Ping::StopApplication (void)
    {
        NS_LOG_FUNCTION (this);
